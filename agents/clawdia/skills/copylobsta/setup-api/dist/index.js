...
app.post("/setup/deploy", requireToken, async (req, res) => {
    const { soulMarkdown, userMarkdown, botUsername, chatId } = req.body;
    if (!soulMarkdown) {
    ...
    const deployAsync = async () => {
        ...
        await runDeployStep("install_deps", async () => {
            ...
            if (!installScript) {
                throw new Error(`Missing install script. Expected one of: ${installScriptCandidates.join(", ")}`);
            }
        ...
        await runDeployStep("configure", async () => {
            ...
            const [telegramToken, anthropicKey, geminiKey, openaiKey] = await Promise.all([
                readSecret("telegram"),
            ...
            if (!telegramToken) {
                throw new Error("Telegram bot token not found in Secrets Manager. " +
