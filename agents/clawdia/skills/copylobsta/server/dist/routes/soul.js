    ...
    try {
        ...
        if (session.groupChatId) {
            ...
            const friendName = session.friendUsername
                ...
                : "Your friend";
            const msg = `${friendName}'s bot ${botName} is live! 🦞\n\n` +
...
function generateSoulDoc(answers) {
    ...
    if (vibe)
        doc += `${name} communicates with a ${vibe} energy. `;
    if (quirks)
        doc += `Quirks: ${quirks}. `;
    if (pushback)
        doc += `On pushback: ${pushback}.`;
    doc += `\n\n`;
    // Relationship Dynamic
    doc += `## Relationship Dynamic\n`;
    doc += `${name}'s role is: ${relationship}.`;
    if (frustrated)
        doc += ` When the user is frustrated: ${frustrated}.`;
    doc += `\n\n`;
    // Boundaries
    if (neverJoke) {
        doc += `## Boundaries\n`;
