...
function generateUserDoc(answers) {
    ...
    if (branch === "student") {
        ...
        if (answers.school)
            doc += ` at ${answers.school}`;
        doc += `.\n\n`;
        // Academics
        doc += `## Academics\n`;
        if (answers.subjects)
            doc += `Current subjects: ${answers.subjects}\n`;
        if (answers.helpSubjects)
            doc += `Needs most help with: ${answers.helpSubjects}\n`;
        if (answers.studyStyle)
            doc += `Study style: ${answers.studyStyle}\n`;
        doc += `\n`;
        // Interests
        ...
        // Preferences
        doc += `## Preferences\n`;
        if (answers.pushOrEasy)
            doc += `- Feedback style: ${answers.pushOrEasy}\n`;
        if (answers.studyStyle)
            doc += `- Study approach: ${answers.studyStyle}\n`;
        doc += `\n`;
    }
    else {
        // Adult path
        ...
        if (answers.occupation)
            doc += ` \u2014 ${answers.occupation}`;
        doc += `.\n\n`;
        // Work & Expertise
        doc += `## Work & Expertise\n`;
        if (answers.botUseCase)
            doc += `Uses bot for: ${answers.botUseCase}\n`;
        if (answers.expertiseAreas)
            doc += `Key topics: ${answers.expertiseAreas}\n`;
        if (answers.skillLevel)
            doc += `Skill level: ${answers.skillLevel}\n`;
        doc += `\n`;
        // Interests
        if (answers.interests) {
            doc += `## Interests\n${answers.interests}\n\n`;
        ...
        // Preferences
        doc += `## Preferences\n`;
        if (answers.learnStyle)
            doc += `- Learning style: ${answers.learnStyle}\n`;
        if (answers.pushOrEasy)
            doc += `- Feedback style: ${answers.pushOrEasy}\n`;
        doc += `\n`;
    }
