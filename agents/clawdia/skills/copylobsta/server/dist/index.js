...
const launchRate = new Map();
function getClientKey(req) {
    return req.ip || req.socket.remoteAddress || "unknown";
}
function checkRateLimit(map, key, now, max) {
...
if (SHARING_ENABLED) {
    ...
    if (!hasStaticTemplate && !hasPresignTemplate) {
        console.error("Missing template source: set COPYLOBSTA_TEMPLATE_S3_BUCKET + COPYLOBSTA_TEMPLATE_S3_KEY (recommended), or CFN_TEMPLATE_URL.");
        process.exit(1);
    }
}
