...
function buildInitData(user, botToken, authDate) {
...
describe("requireTelegramUser", () => {
    ...
    it("throws on missing hash", () => {
        ...
        expect(() => requireTelegramUser(params.toString(), BOT_TOKEN)).toThrow("missing hash");
    });
    it("throws on invalid signature", () => {
        const initData = buildInitData({ id: 1, first_name: "X" }, BOT_TOKEN);
        const wrongToken = "999999999:WRONGtokenWRONGtokenWRONGtokenWRON";
        expect(() => requireTelegramUser(initData, wrongToken)).toThrow("invalid initData signature");
    });
    ...
    it("accepts initData within custom maxAge", () => {
        ...
        const initData = buildInitData({ id: 99, first_name: "Recent" }, BOT_TOKEN, recentDate);
        const result = requireTelegramUser(initData, BOT_TOKEN, {
            maxAgeSeconds: 120,
        });
...
//# sourceMappingURL=telegramAuth.test.js.map