...
export function canTransition(from, to) {
    ...
    if (to === "PAUSED" || to === "FAILED" || to === "ABANDONED")
        return true;
...
export function getNextState(current) {
    ...
    if (idx === -1 || idx >= HAPPY_PATH.length - 1)
        return null;
    return HAPPY_PATH[idx + 1];
}
/** Transition a session to a new state. Validates the transition and persists. */
export function transition(session, targetState) {
    // PAUSED can only resume to its previous state
    if (session.state === "PAUSED") {
        if (targetState !== session.previousState) {
            throw new Error(`PAUSED sessions can only resume to previous state (${session.previousState}), not ${targetState}`);
        }
    }
    else if (!canTransition(session.state, targetState)) {
        throw new Error(`Invalid transition: ${session.state} → ${targetState}`);
    }
    const patch = {
        state: targetState,
    };
