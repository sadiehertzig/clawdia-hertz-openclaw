 ...
 */
export async function sendLauncherButton(chatId, startParam, miniAppBaseUrl, userId) {
    ...
    if (!res.ok) {
        ...
        if (body.includes("BUTTON_TYPE_INVALID")) {
            ...
            if (userId) {
                ...
                if (!dmRes.ok) {
                    ...
                    if (dmBody.includes("chat not found") || dmBody.includes("bot can't initiate")) {
                        ...
                        await sendMessage(chatId, "I need to send you the setup link in a DM, but it looks like we haven't chatted before!\n\n" +
            ...
            await sendMessage(chatId, "CopyLobsta setup needs to open in a DM. Send me /copylobsta in a direct message!");
