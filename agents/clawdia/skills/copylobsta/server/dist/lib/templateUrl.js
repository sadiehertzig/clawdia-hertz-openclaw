...
import { CFN_TEMPLATE_URL, TEMPLATE_S3_BUCKET, TEMPLATE_S3_KEY, TEMPLATE_S3_REGION, TEMPLATE_URL_TTL_SECONDS, } from "../config.js";
const MIN_TTL_SECONDS = 60;
const MAX_TTL_SECONDS = 3600;
function getPresignTtlSeconds() {
    if (!Number.isFinite(TEMPLATE_URL_TTL_SECONDS))
        return 600;
...
export async function resolveTemplateUrl() {
    ...
    if (hasBucket || hasKey) {
        if (!hasBucket || !hasKey) {
            throw new Error("Template source misconfigured: set both COPYLOBSTA_TEMPLATE_S3_BUCKET and COPYLOBSTA_TEMPLATE_S3_KEY, or use CFN_TEMPLATE_URL fallback.");
        }
    ...
    if (!CFN_TEMPLATE_URL) {
        throw new Error("Template source is not configured. Set COPYLOBSTA_TEMPLATE_S3_BUCKET + COPYLOBSTA_TEMPLATE_S3_KEY (recommended), or CFN_TEMPLATE_URL.");
    }
